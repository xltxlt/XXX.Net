      ...
      <section class="canvas-panel" @click.self="selectedId = ''" @dragover.prevent @drop="dropOnRoot">
        ...
        <div v-else class="canvas-items">
          <template v-for="item in form.form" :key="item.ident">
            <div v-if="isContainer(item)" class="canvas-group" :class="{ selected: selectedId === item.ident }"
              @click.stop="select(item)" @dragover.prevent @drop.stop="dropInContainer($event, item)">
          ...
          <el-form label-position="top" size="default">
            ...
            <el-form-item v-if="!isContainer(selectedItem)" label="自定义属性">
              <el-input
                ...
                type="textarea"
                :rows="8"
                placeholder='例如：{"showLabel":false,"labelWidth":120}'
...
const customAttrText = computed(() => {
  if (!selectedId.value) return '{}'
  return JSON.stringify(attrData.value[selectedId.value] ?? {}, null, 2)
})
...
const addGroup = () => { const item: DesignerItem = { ident: nextId(), formType: PageFormGroup.Group, label: '基础信息', fieldName: '', child: [] }; attrData.value[item.ident] = {}; form.form.push(item as never); select(item) }
const addTable = () => {
  ...
  attrData.value[item.ident] = {}; form.form.push(item as never); select(item)
}
const addList = () => {
...
const takeDraggedItem = (): DesignerItem | undefined => {
  if (draggingPalette.value) {
    ...
    draggingPalette.value = undefined
    if (palette.formType === PageFormGroup.Group) { const item = { ident: nextId(), formType: PageFormGroup.Group, label: palette.label, fieldName: '', child: [] } as DesignerItem; attrData.value[item.ident] = {}; return item }
    if (palette.formType === PageFormGroup.Table) { const item = { ident: nextId(), formType: PageFormGroup.Table, label: palette.label, fieldName: `details${id}`, child: [{ ident: nextId(), formType: PageFormGroup.Group, label: '', fieldName: '', child: [] }] } as DesignerItem; attrData.value[item.ident] = {}; return item }
    if (palette.formType === PageFormGroup.List) { const item = { ident: nextId(), formType: PageFormGroup.List, label: palette.label, fieldName: `list${id}`, child: [{ ident: nextId(), formType: PageFormGroup.Group, label: '', fieldName: '', child: [] }] } as DesignerItem; attrData.value[item.ident] = {}; return item }
    const item = createField(palette); attrData.value[item.ident] = {}; return item
  }
  const ident = draggingItemId.value
  draggingItemId.value = ''
  return ident ? detachItem(form.form as DesignerItem[], ident) : undefined
}
const dropOnRoot = () => { const item = takeDraggedItem(); if (item) { form.form.push(item as never); select(item) } }
...
const removeSelected = () => {
  const result = parentAndIndex(form.form, selectedId.value)
  if (result) result.items.splice(result.index, 1)
  if (selectedId.value) delete attrData.value[selectedId.value]
  selectedId.value = ''
}
const optionsFromItem = (item: DesignerItem) => (item.optionText ?? '').split('\n').map((line) => line.trim()).filter(Boolean).map((line) => { const [label, ...rest] = line.split(','); return { label: label.trim(), value: (rest.join(',').trim() || label.trim()) } })
...
const showJson = () => { jsonText.value = JSON.stringify({ ...exportData(), attrData: attrData.value }, null, 2); jsonVisible.value = true }
const resetDesigner = () => { form.form.splice(0); attrData.value = {}; selectedId.value = '' }
const updateCustomAttr = (value: string) => {
  if (!selectedId.value) return
  try {
    const parsed = JSON.parse(value || '{}')
    if (!parsed || Array.isArray(parsed) || typeof parsed !== 'object') throw new Error('属性必须是 JSON 对象')
    attrData.value[selectedId.value] = parsed
  } catch {
    ElMessage.warning('自定义属性必须是合法的 JSON 对象')
  }
}
...
const parseFormJson = (value: unknown): any => {
  if (!value) return {}
