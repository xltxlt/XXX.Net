...
const pageConfig: TempListPageConfig = {
  url: dictTypeService.apiSysDictTypePagelistPost.bind(dictTypeService),
  optionUrl: dictTypeService.apiSysDictTypePageoptionGet.bind(dictTypeService),
  pageFun: {
    add: (data: any) => {
      return {
        title: "新增",
    ...
    batchDel: async (data: any[]) => {
      ...
      const res = await dictTypeService.apiSysDictTypeBatchlogicdeletePost(ids);
    ...
    del: async (data: any) => {
      ...
      const res = await dictTypeService.apiSysDictTypeLogicdeleteIdPost(data.id);
      handleSumbitResBox(res, '删除成功', () => {
        pageInfoRef.value?.onRefresh();
      });
    },
