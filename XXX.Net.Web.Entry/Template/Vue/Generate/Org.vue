...
const pageConfig: TempListPageConfig = {
  url: orgService.apiSysOrgPagelistPost.bind(orgService),
  optionUrl: orgService.apiSysOrgPageoptionGet.bind(orgService),
  pageFun: {
    add: (data: any) => {
      return {
        title: "新增",
    ...
    batchDel: async (data: any[]) => {
      ...
      const res = await orgService.apiSysOrgBatchlogicdeletePost(ids);
    ...
    del: async (data: any) => {
      ...
      const res = await orgService.apiSysOrgLogicdeleteIdPost(data.id);
      handleSumbitResBox(res, '删除成功', () => {
        pageInfoRef.value?.onRefresh();
      });
    },
