...
import { inventoryTypeService } from "@/api/index.ts";
import inventoryTypeEdit from "./inventoryTypeEdit.vue";
...
const pageConfig: TempListPageConfig = {
  url: inventoryTypeService.apiInventoryTypePagelistPost.bind(inventoryTypeService),
  optionUrl: inventoryTypeService.apiInventoryTypePageoptionGet.bind(inventoryTypeService),
  pageFun: {
    add: (data: any) => {
      return {
        title: "新增",
        comp: inventoryTypeEdit,
        height: "70%",
        pars: {},
      };
    },
    ...
    batchDel: async (data: any[]) => {
      ...
      const res = await inventoryTypeService.apiInventoryTypeBatchlogicdeletePost(ids);
    ...
    del: async (data: any) => {
      ...
      const res = await inventoryTypeService.apiInventoryTypeLogicdeleteIdPost(data.id);
      handleSumbitResBox(res, '删除成功', () => {
        pageInfoRef.value?.onRefresh();
      });
    },
