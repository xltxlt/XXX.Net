...
import inventoryLocationEdit from "./inventoryLocationEdit.vue";
...
const pageConfig: TempListPageConfig = {
  url: inventoryLocationService.apiInventoryLocationPagelistPost.bind(inventoryLocationService),
  optionUrl: inventoryLocationService.apiInventoryLocationPageoptionGet.bind(inventoryLocationService),
  pageFun: {
    add: (data: any) => {
      return {
        title: "新增",
        comp: inventoryLocationEdit,
        height: "70%",
        pars: {},
      };
    },
    ...
    batchDel: async (data: any[]) => {
      ...
      const res = await inventoryLocationService.apiInventoryLocationBatchlogicdeletePost(ids);
    ...
    del: async (data: any) => {
      ...
      const res = await inventoryLocationService.apiInventoryLocationLogicdeleteIdPost(data.id);
      handleSumbitResBox(res, '删除成功', () => {
        pageInfoRef.value?.onRefresh();
      });
    },
