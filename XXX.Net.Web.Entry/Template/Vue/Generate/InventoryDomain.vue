...
import { inventoryDomainService } from "@/api/index.ts";
import inventoryDomainEdit from "./inventoryDomainEdit.vue";
...
const pageConfig: TempListPageConfig = {
  url: inventoryDomainService.apiInventoryDomainPagelistPost.bind(inventoryDomainService),
  optionUrl: inventoryDomainService.apiInventoryDomainPageoptionGet.bind(inventoryDomainService),
  pageFun: {
    add: (data: any) => {
      return {
        title: "新增",
        comp: inventoryDomainEdit,
        height: "70%",
        pars: {},
      };
    },
    ...
    batchDel: async (data: any[]) => {
      ...
      const res = await inventoryDomainService.apiInventoryDomainBatchlogicdeletePost(ids);
    ...
    del: async (data: any) => {
      ...
      const res = await inventoryDomainService.apiInventoryDomainLogicdeleteIdPost(data.id);
      handleSumbitResBox(res, '删除成功', () => {
        pageInfoRef.value?.onRefresh();
      });
    },
